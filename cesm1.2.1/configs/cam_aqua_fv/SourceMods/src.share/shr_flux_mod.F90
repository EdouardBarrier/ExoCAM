...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/release_tags/cesm1_2_x_n00_share3_130528/shr/shr_flux_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
! !EOP
   ...
   real(R8)    :: P      ! dummy arg ~ pressure (Pa)
   ...
   qsat(Tk, P) =   (shr_const_mwwv/shr_const_mwdair)*(611.2_r8*exp((17.67_r8*(Tk-273.15_R8))/(Tk-29.65_r8))) /&
                   (P - (611.2_r8*exp((17.67_r8*(Tk-273.15_R8))/(Tk-29.65_r8)))*(1-(shr_const_mwwv/shr_const_mwdair)))
...
!-------------------------------------------------------------------------------
   ...
   DO n=1,nMax
     if (mask(n) /= 0) then
        ...
        pbot   = rbot(n) * tbot(n) * shr_const_rdair !need to use dry rgas (and also, maybe still recover dry pressure?)
        vmag   = max(umin, sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2) )
        thvbot = thbot(n) * (1.0_R8 + shr_const_zvir * qbot(n)) ! virtual temp (K)
        ssq    = 0.98_R8 * qsat(ts(n),pbot)        ! sea surf hum (kg/kg)
        ssq    = min(0.99_r8, ssq)
      ...
      !   if (n==1) then
      !      write(s_logunit,*) "Sea Surface T:", ts(n)
      !      write(s_logunit,*) "Atmospheric bottom layer T:", tbot(n)
      !      !write(s_logunit,*) "Atmospheric bottom layer Theta:", thbot(n)
      !      !write(s_logunit,*) "tstar:", tstar, "ustar:", ustar
      !      !write(s_logunit,*) "cp:", cp, "tau:", tau
      !      !write(s_logunit,*) "shr_const_cpvir:", shr_const_cpvir, "ssq:", ssq
      !    !   write(s_logunit,*) "latent heat flux:", lat(n)
      !    !   write(s_logunit,*) "sensible heat flux:", sen(n)
      !    !   write(s_logunit,*) "lwup:", lwup(n)
      !      write(s_logunit,*) "water flux:", evap(n)
      !      write(s_logunit,*) "qstar:", qstar, "re:", re, "delq:", delq
      !      write(s_logunit,*) "qbot:", qbot(n), "ssq:", ssq, "rbot:", rbot(n), "qsat:", qsat(ts(n),pbot), "pbot:", pbot
      !      write(s_logunit,*) " "
      !   end if
