...
!
   ...
   real(r8) :: rcpres         ! reciprocal of cp
...
!
   ...
   real(r8) cpres          ! specific heat of dry air
...
!
   cpres     = cpair         ! specific heat of dry air
...
!
   ...
   rcpres    = 1.0_r8/cpres        ! reciprocal specific heat of dry air
...
!#######################################################################
   ...
   use zm_conv, only: entropy, ientropy
   use shr_const_mod, only: shr_const_mwwv, shr_const_mwdair, shr_const_cpwv
...
!  JJH extension to facilitate export of cloud liquid water
   ...
   real(r8) tv(pcols,pver)      ! virtual temperature in layer
   real(r8) mu_red              ! reduced molecular mass
   real(r8) etac(pcols)         ! entropy of rising plume
   real(r8) cpc(pcols)          ! heat capacity of rising plume
   logical do_moist_conv(pcols)
   logical do_dry_conv(pcols)
   real(r8) tpv                 ! virtual temp of parcel
   real(r8) tp                  ! temp of parcel from ientropy
   real(r8) qst                 ! qsat post ientropy
   real(r8) qv                  ! vapour water MMR from ientropy
   real(r8) qcond                  ! condensed water MMR from ientropy
   real(r8) cpmix               !cpmix in layer
   real(r8) cpmixp1             !cpmix in layer below
   real(r8) cpmixm1             !cpmix in layer above
   real(r8) c0_mod(pcols)           !modified c0 based on local mean molecular weight

   integer indx2(pcols)     ! longitude indices for condition true (dry convection)
   integer len2             ! vector length of "gathered" vectors (dry convection)
...
!
   do k=limcnv,pver
      do i=1,ncol
         cpmix = (1-shb(i,k))*cpres + shb(i,k)*shr_const_cpwv
         sb (i,k) = cpmix*tb(i,k) + zm(i,k)*grav + phis(i)
...
!
         ...
         if ( (zm(i,k+1) <= pblhgt) .and. dzcld(i) == 0.0_r8 ) then
            ...
            qsattp = shbs(i,k+1) + cpres*rhlat*gam(i,k+1)*tprime !keep cpd here as just reciprocal of gam which also has cpd
...
!
         cpmix = (1-shb(i,k+1))*cpres + shb(i,k+1)*shr_const_cpwv
         shc(i)    = shb(i,k+1) + qprime
         sc (i)    = cpmix*(tb(i,k+1)+tprime) + zm(i,k+1)*grav + phis(i)
         hc (i)    = sc (i    ) + hlat*shc(i)

         call entropy(tb(i,k+1)+tprime, pmid(i,k+1)*0.01_r8, shb(i,k+1)+qprime, etac(i)) !entropy and ientropy expect pressure in mb not Pa
         call ientropy(9, i, lchnk, etac(i), pmid(i,k)*0.01_r8, shb(i,k+1)+qprime, tp, qst, tb(i,k), qv, qcond)
         tpv = tp * (1 - mu_red*qv)
         cldwtr(i) = hlat * qcond  !Ll, may as well set here - 0 if no condensation
         ! reset these to false
         do_dry_conv(i) = .false.
         do_moist_conv(i) = .false.
         if ((tpv > tv(i,k)) .and. (qcond>0._r8)) then
            do_moist_conv(i) = .true.
         else if ((tpv > tv(i,k)) .and. (qcond==0._r8)) then
            do_dry_conv(i) = .true.
            ! if (i==1) write(iulog,*) "k:", k, "tpv:", tpv, "tv:", tv(i,k), "sc:", sc(i), "s:", sb(i,k)
         end if

         ! if (i==1) then
         ! write(iulog,*) " "
         ! write(iulog,*) "HK_CONV. i:", i, "middle k:", k
         ! write(iulog,*) "PBL pertubations. tprime:", tprime, "qprime:", qprime
         ! write(iulog,*) "layer below. p:", pmid(i,k+1), "t:", tb(i,k+1)+tprime, "q:", shb(i,k+1)+qprime, "sc:", sc(i)
         ! write(iulog,*) "middle layer. p:", pmid(i,k), "t:", tb(i,k), "q:", shb(i,k), "s:", sb(i,k) 
         ! write(iulog,*) "middle layer. env tv:", tv(i,k), "parcel tv:", tpv, "qcond:", qcond, "tp:", tp, "qv:", qv
         ! write(iulog,*) "do dry convection:", do_dry_conv(i), "do moist convection:", do_moist_conv(i)
         ! write(iulog,*) " "
         ! end if

         vtemp4(i) = hc(i) - hbs(i,k)
         dz        = pdel(i,k)*rgas*tb(i,k)*rgrav/pmid(i,k)
         if (do_moist_conv(i) .or. do_dry_conv(i)) then
            dzcld(i) = dzcld(i) + dz
         else
            dzcld(i) = 0.0_r8
         end if
         c0_mod(i) = c0 * (shr_const_mwdair/(1-mu_red*shc(i))/29) !reduces autoconversion so it's reasonably constant in pressure terms across different atmospheres
10       continue
#if ( defined DIAGNS )
!#######################################################################
!#                                                                     #
...
!
         ...
         len2 = 0
         do i=1,ncol
            if (do_moist_conv(i)) then
               len1 = len1 + 1
               indx1(len1) = i
            end if
            if (do_dry_conv(i)) then
               len2 = len2 + 1
               indx2(len2) = i
            end if
         end do
         if ((len1 <= 0) .and. (len2 <= 0)) go to 70
!
...
!
         if (k <= limcnv+1) then
            ...
            do ii=1,len2  !DRY CONVECTION ADDITION
               i = indx2(ii)
               !set beta for level just below top level, some associated quantities
               beta(i) = 0.0_r8 !cannot go into top layer of triplet
               vtemp3(i) = sc(i) - sbh(i,k) !dry convection version of variable
            end do
         else
!
...
!DIR$ CONCURRENT
            do ii=1,len1
               i = indx1(ii)
               betamx(i) = 1.0_r8 - c0_mod(i)*max(0.0_r8,(dzcld(i)-dzmin))
...
!
               ...
               if (hc(i) - hbs(i,k) > 0._r8) then
                  if ((beta(i)*vtemp2(i) - vtemp1(i)) > 0._r8) then
                     betamx(i) = 0.99_r8*(vtemp1(i)/vtemp2(i))
                     beta(i)   = max(0.0_r8,min(betamx(i),beta(i)))
                  end if
               else !convection may still be favoured bc of compositional differences etc
                  if ((beta(i)*vtemp2(i) - vtemp1(i)) < 0._r8) then
                     if (vtemp2(i) < 0._r8) then
                        betamx(i) = 0.99_r8*(vtemp1(i)/vtemp2(i))
                     else
                        betamx(i) = 1.01_r8*(vtemp1(i)/vtemp2(i)) !think this is right condition - want the total to be just > 0
                     end if
                  end if
               end if
            end do
            ...
            do ii=1,len2  !DRY CONVECTION ADDITION
               i = indx2(ii)
               ! first guess at beta with buoyancy argument, existing maxima and minima, constrain to eta stays positive
               betamx(i) = 1.0_r8 - c0_mod(i)*max(0.0_r8,(dzcld(i)-dzmin)) !do we keep this in? it's based off autoconversion of rainwater, but we have none
               b1 = (sc(i) - sb(i,k-1))*pdel(i,k-1) !modified to use s instead of h*
               b2 = (sc(i) - sb(i,k  ))*pdel(i,k  ) !modified to use s instead of h*
               beta(i)   = max(betamn,min(betamx(i), 1.0_r8 + b1/b2))
               if (hbs(i,k-1) <= hb(i,k-1)) beta(i) = 0.0_r8 !if randomly have saturated k-1 do not extend plume - may as well keep in

               ! if (i==1) then
               !    write(iulog,*) "k:", k, "1st beta guess:", beta(i), "betamx:", betamx(i), "buoyancy estimate:", 1.0_r8 + b1/b2
               !    ! write(iulog,*) "b1:", b1, "b2:", b2, "buoyancy estimate:", 1.0_r8 + b1/b2
               ! end if
               !bound maximum beta so eta remains positive (assuming eta positive for beta=0)
               vtemp1(i) = (sc(i) - sbh(i,k+1)) - (sbh(i,k+1) - sc(i))*pdel(i,k)*rpdel(i,k+1)
               vtemp2(i) = sc(i) - sbh(i,k)
               vtemp3(i) = vtemp2(i)

               if ((sc(i)-sb(i,k)) > 0._r8) then
                  if ((beta(i)*vtemp2(i) - vtemp1(i)) > 0._r8) then
                     betamx(i) = 0.99_r8*(vtemp1(i)/vtemp2(i))
                     beta(i)   = max(0.0_r8,min(betamx(i),beta(i)))
                  end if
               ...
               ! if (i==1) then
               !    write(iulog,*) "k:", k, "2nd beta guess (+ve eta):", beta(i), "vtemp1:", vtemp1(i), "vtemp2:", vtemp2(i)
               ! end if
            end do
!
...
!DIR$ CONCURRENT
            ...
            ! we do not impose an additional constraint for the dry convection because we are not worried about such supersaturation
!
...
!DIR$ CONCURRENT
            ...
            do ii=1,len2  !DRY CONVECTION ADDITION
               i = indx2(ii)
               ! constrain beta to avoid introducing thermodynamic noise in the vertical
               g = min(0.0_r8,sb(i,k) - sb(i,k-1)) !margin allowed
               temp1 = (sb(i,k) - sb(i,k-1) - g)*(cats/ztodt)/(sc(i) - sb(i,k))
               vtemp1(i) = temp1*vtemp1(i)*rpdel(i,k) + (sc(i) - sbh(i,k+1))*rpdel(i,k)
               vtemp2(i) = temp1*vtemp3(i)*rpdel(i,k) + (sc(i) - sbh(i,k))* &
                           (rpdel(i,k) + rpdel(i,k+1))
               if ((beta(i)*vtemp2(i) - vtemp1(i)) > 0._r8) then
                  if (vtemp2(i) /= 0.0_r8) then
                     betamx(i) = vtemp1(i)/vtemp2(i)
                  ...
                  beta(i) = max(0.0_r8,min(betamx(i),beta(i)))
                  ! if (i==1) then
                  !    write(iulog,*) "k:", k, "change on 3rd beta guess (thermo noise):", beta(i)
                  ! end if
               end if
            end do
...
!DIR$ CONCURRENT
         do ii=1,len1
            ...
            if (eta(i) > tmass*rtdt) eta(i) = 0.99_r8 * tmass*rtdt !!! EDITED
            if (eta(i) <= 0.0_r8) eta(i) = 0.0_r8
!
...
!
         ...
         do ii=1,len2  !DRY CONVECTION ADDITION
            i = indx2(ii)
            ! calculate mass flux mc. impose various additional checks and constraints as needed above
            beta(i) = max(0.0_r8,beta(i))
            temp1 = sc(i) - sb(i,k)
            temp2 = ((sc(i) - sbh(i,k+1)) - &
                        beta(i)*vtemp3(i))*rpdel(i,k) - (sbh(i,k+1) - sc(i))*rpdel(i,k+1)
            eta(i) = temp1/(temp2*grav*cats)
            tmass = min(pdel(i,k),pdel(i,k+1))*rgrav
            ! if (i==1) then
            !    write(iulog,*) "k:", k, "1st eta guess:", eta(i)
            !    write(iulog,*) "temp1:", temp1, "temp2:", temp2, "gT:", grav*cats, "beta:", beta(i)
            !    write(iulog,*) " "
            ! end if
            if (eta(i) > tmass*rtdt) eta(i) = 0.99_r8 * tmass*rtdt !completely block convection is it would be too strong?
            if (eta(i) <= 0.0_r8) eta(i) = 0.0_r8
            ! if (i==1) then
            !    write(iulog,*) "k:", k, "2nd eta guess:", eta(i), "max allowed:", tmass*rtdt
            ! end if
            
            ! Check on negative q in top layer (bound beta) - unobjectionable, don't think it needs to change
            if (shc(i)-shbh(i,k) < 0.0_r8 .and. beta(i)*eta(i) /= 0.0_r8) then
               denom = eta(i)*grav*ztodt*(shc(i) - shbh(i,k))*rpdel(i,k-1)
               beta(i) = max(0.0_r8,min(-0.999_r8*shb(i,k-1)/denom,beta(i)))
               ! if (i==1) then
               !    write(iulog,*) "k:", k, "negative q in top layer, beta:", beta(i)
               ! end if
            end if
            
            ! Check on negative q in middle layer (zero eta) - not quite sure why we set to 0, but stick with it
            qtest1 = shb(i,k) + eta(i)*grav*ztodt*((shc(i) - shbh(i,k+1)) - &
                     (1.0_r8 - beta(i))*cldwtr(i)*rhlat - beta(i)*(shc(i) - shbh(i,k)))* &
                  rpdel(i,k)
            if (qtest1 <= 0.0_r8) eta(i) = 0.0_r8
            ! if (i==1) then
            !    write(iulog,*) "k:", k, "check on negative q in middle layer, eta:", eta(i)
            ! end if

            ! Check on negative q in lower layer (bound eta) - stick with this as well
            fac1 = -(shbh(i,k+1) - shc(i))*rpdel(i,k+1)
            qtest2 = shb(i,k+1) - eta(i)*grav*ztodt*fac1
            if (qtest2 < 0.0_r8) then
               eta(i) = 0.99_r8*shb(i,k+1)/(grav*ztodt*fac1)
            ...
            ! if (i==1) then
            !    write(iulog,*) "k:", k, "check on negative q in lower layer, eta:", eta(i)
            ! end if
            etagdt(i) = eta(i)*grav*ztodt

            ! if (i==1) then
            !    write(iulog,*) "k:", k, "beta:", beta(i), "etagdt:", etagdt(i)
            !    write(iulog,*) " "
            ! end if
...
! Need to modify ds1 (a quantity in joules) by the LOCAL heat capacity - should we be including updated q in this?
!
            cpmixp1 = (1-shb(i,k+1))*cpres + shb(i,k+1)*shr_const_cpwv
            cpmix = (1-shb(i,k))*cpres + shb(i,k)*shr_const_cpwv
            cpmixm1 = (1-shb(i,k-1))*cpres + shb(i,k-1)*shr_const_cpwv
            tb (i,k+1) = tb(i,k+1)  + ds1(i)/cpmixp1
            tb (i,k  ) = tb(i,k  )  + ds2(i)/cpmix
            tb (i,k-1) = tb(i,k-1)  + ds3(i)/cpmixm1
...
30          continue
         ...
         do ii=1,len2  !DRY CONVECTION ADDITION
            i = indx2(ii)
            ! calculate cloud water, rainwater, thermodynamic changes - there is no cloudwater, no rainwater etc
            ! isolate thermodynamic fluxes, update diagnostic information
            icwmr(i,k) = 0.0_r8
            cldwtr(i) = 0.0_r8
            ! JJH changes to facilitate export of cloud liquid water --------------------------------
            totcond(i) = 0.0_r8
            ...
            ds1(i) = etagdt(i)*(sbh(i,k+1) - sc(i))*rpdel(i,k+1)
            dq1(i) = etagdt(i)*(shbh(i,k+1) - shc(i))*rpdel(i,k+1)
            ds2(i) = (etagdt(i)*(sc(i) - sbh(i,k+1)) - beta(i)*etagdt(i)*(sc(i) - sbh(i,k)))*rpdel(i,k)
            ! JJH change for export of cloud liquid water; must use total condensate 
            ! since rainwater no longer represents total condensate
            dq2(i) = (etagdt(i)*(shc(i) - shbh(i,k+1)) - beta(i) * etagdt(i)*(shc(i) - shbh(i,k)))*rpdel(i,k)
            ds3(i) = beta(i)*(etagdt(i)*(sc(i) - sbh(i,k)))*rpdel(i,k-1)
            dq3(i) = beta(i)*etagdt(i)*(shc(i) - shbh(i,k))*rpdel(i,k-1)
            !
            ! Isolate convective fluxes for later diagnostics
            fslkp = eta(i)*(sc(i) - sbh(i,k+1))
            ...
            ! if (i==1) then
            !    write(iulog,*) " "
            !    write(iulog,*) "k:", k, "beta:", beta(i), "eta:", eta(i)
            !    write(iulog,*) "old s k+1:", sb(i,k+1), "new s k+1:", sb(i,k+1)+ds1(i)
            !    write(iulog,*) "old s k:  ", sb(i,k), "new s k:  ", sb(i,k)+ds2(i)
            !    write(iulog,*) "old s k-1:", sb(i,k-1), "new s k-1:", sb(i,k-1)+ds3(i)
            !    write(iulog,*) "old q k+1:", shb(i,k+1), "new q k+1:", shb(i,k+1)+dq1(i)
            !    write(iulog,*) "old q k:  ", shb(i,k), "new q k:  ", shb(i,k)+dq2(i)
            !    write(iulog,*) "old q k-1:", shb(i,k-1), "new q k-1:", shb(i,k-1)+dq3(i)
            ! end if
            if (dq1(i) > 0.1_r8) write(iulog,*) "lchnk:", lchnk, "i:", i, "k:", k+1, "dq1:", dq1(i)
            if (dq2(i) > 0.1_r8) write(iulog,*) "lchnk:", lchnk, "i:", i, "k:", k  , "dq2:", dq2(i)
            if (dq3(i) > 0.1_r8) write(iulog,*) "lchnk:", lchnk, "i:", i, "k:", k-1, "dq3:", dq3(i)
...
40                continue
               ...
               do 80 ii=1,len2
                  i = indx2(ii)  !DRY CONVECTION ADDITION
                  ! convectively transport trace consitutents
                  !
                  ! If any of the reported values of the constituent is negative in
                  ! the three adjacent levels, nothing will be done to the profile
                  if ((dq(i,k+1,m) < 0.0_r8) .or. (dq(i,k,m) < 0.0_r8) .or. (dq(i,k-1,m) < 0.0_r8)) go to 80
                  !
                  ! Specify constituent interface values (linear interpolation)
                  cmrh(i,k  ) = 0.5_r8*(dq(i,k-1,m) + dq(i,k  ,m))
                  cmrh(i,k+1) = 0.5_r8*(dq(i,k  ,m) + dq(i,k+1,m))
...
!DIR$ CONCURRENT
                  ...
                  do ii=1,len2  !DRY CONVECTION ADDITION
                     i = indx2(ii)
                     vtemp1(ii     ) = tb(i,k)
                     vtemp1(ii+len2) = tb(i,k-1)
                     vtemp2(ii     ) = pmid(i,k)
                     vtemp2(ii+len2) = pmid(i,k-1)
                  end do

                  call qsat(vtemp1(:2*len2), vtemp2(:2*len2), &
                        vtemp5(:2*len2), vtemp3(:2*len2), gam=vtemp4(:2*len2))

                  do ii=1,len2  !DRY CONVECTION ADDITION
                     i = indx2(ii)
                     ! complete update of thermodynamic structure
                     shbs(i,k  ) = vtemp3(ii     )
                     shbs(i,k-1) = vtemp3(ii+len2)
                     gam(i,k  ) = vtemp4(ii     )
                     gam(i,k-1) = vtemp4(ii+len2)
                     sb (i,k  ) = sb(i,k  ) + ds2(i)
                     sb (i,k-1) = sb(i,k-1) + ds3(i)
                     hb (i,k  ) = sb(i,k  ) + hlat*shb(i,k  )
                     hb (i,k-1) = sb(i,k-1) + hlat*shb(i,k-1)
...
!DIR$ CONCURRENT
                  ...
                  do ii=1,len2  !DRY CONVECTION ADDITION
                     i = indx2(ii)
                     !update thermodynamic information at half (interface) levels
                     sbh (i,k) = 0.5_r8*(sb(i,k) + sb(i,k-1))
                     shbh(i,k) = qhalf(shb(i,k-1),shb(i,k),shbs(i,k-1),shbs(i,k))
                     hbh (i,k) = sbh(i,k) + hlat*shbh(i,k)
                     sbh (i,k-1) = 0.5_r8*(sb(i,k-1) + sb(i,k-2))
...
!**BAB don't update input profiles
               ...
               !    if (i==1) then
               !       write(iulog,*) " "
               !       write(iulog,*) "lchnk:", lchnk, "i:", i, "Hack Shallow convection"
               !       do k=pver,25, -1
               !          write(iulog,*) "k:", k, "old s:", sb(i,k)/cpres, "new s:", (sb(i,k)+cmfdt(i,k)/rtdt/adjfac)/cpres, "change:", cmfdt(i,k)/rtdt/adjfac/cpres
               !       end do
               !    end if
               ! end do
