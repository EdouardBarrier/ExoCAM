...
! !LOCAL VARIABLES:
      ...
      do k=1,km
         do i=i1,i2
            ...
            !write(iulog,*) "k:", k, "i:", i, "j:", j, "q1:", q1(i,j,k), "q4:", q4(1,i,k)
            q4(1,i,k) = q1(i,j,k)
         enddo
      enddo
...
!dir$ prefervector
      ...
      do i = i1, i2
         ! if (i==1) then
         !    write(iulog,*) "kn:", kn, "km:", km
         !    write(iulog,*) "pe1:", pe1(i,:)
         !    write(iulog,*) "pe2:", pe2(i,:)
         ! end if
         k0(i,0) = 1
         do k = 1, kn+1
            k0found = -1
            do kk = k0(i,k-1), km
               ! if (i==1 .and. (k==52 .and. kk==51) ) then
               !    write(iulog,*) "k:", k, "kk:", kk, "pe2:", pe2(i,k), "pe1:", pe1(i,kk+1)
               ! end if
               if (pe2(i,k) <= pe1(i,kk+1)) then
                  k0(i,k) = kk
                  k0found = kk
            ...
            if (k0found .lt. 0) then
               write(iulog,*) 'mapz error - k0found i j k (kk,pe1,pe2) = ',   &
                  k0found, i, j, k, kk,pe1(i,kk),pe2(i,kk)
