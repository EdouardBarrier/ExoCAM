module hb_diff
  ...
  real(r8), parameter :: zkmin     = 0.08_r8        ! Minimum kneutral*f(ri)
  !
  ! PBL Parameters
  !
...
!===============================================================================
  ...
  subroutine compute_hb_diff(lchnk, ncol,            &
    ...
    use physconst,       only: cpair, cpwv, rair
    ...
    ! virtual temperature - new method needed to account for compositional effects, copied from zm_conv
   ...
   do k=pver,ntop_turb, -1
      do i=1,ncol
         !find new beta_mix_integral (L17 or B24)
         cp_mix = ((1-q(i,k))*cpair + q(i,k)*cpwv)
         beta_mix = (rair*(1 - mu_red*q(i,k))/cp_mix)
         if (k==pver) then
            beta_mix_integral(i) = beta_mix_integral(i) + beta_mix * log(pint(i,k+1)/pmid(i,k)) !need to start somewhere so start at very model bottom
         else
            beta_mix_integral(i) = beta_mix_integral(i) + beta_mix * log(pmid(i,k+1)/pmid(i,k))
         end if
         !use this to evaluate the virtual potential temperature
         thv(i,k) = t(i,k) * (1 - mu_red*q(i,k)) * exp(beta_mix_integral(i))
      end do
   end do
  ...
  subroutine trbintd(ncol    ,                            &
    ...
    do k=ntop_turb,nbot_turb-1
       do i=1,ncol
         ...
         !  if (i==1) then
         !    write(iulog,*) "i:", i, "k:", k, "Ri:", ri(i,k), "s2:", s2(i,k), "n2:", n2(i,k), "thv:", thv(i,k)
         !    write(iulog,*) "dvdz2:", dvdz2, "dz:", dz, "du:",(u(i,k)-u(i,k+1)), "dv:", (v(i,k)-v(i,k+1)), "dthetav:", ( thv(i,k) - thv(i,k+1))
         !    write(iulog,*) " "
         !  end if
       end do
    end do
  ...
  subroutine austausch_atm(ncol    ,ri      ,s2      ,kvf     )
    ...
    do k = ntop_turb, nbot_turb-1
       do i=1,ncol
         ...
         !  if (i==1) then
         !    write(iulog,*) "i:", i, "k:", k
         !    write(iulog,*) "kvf:", kvf(i,k+1), "fofri:", fofri, "ml2:", ml2(k), "S:", sqrt(s2(i,k)), "Ri:", ri(i,k)
         !    write(iulog,*) " "
         !  end if
       end do
    end do
