...
!   15.05.01   Bardeen    Updated dyn/phys stepping 
!
!EOP
!-----------------------------------------------------------------------
...
! Variables from the dynamics import container
    ...
    real(r8), pointer :: tracer(:,:,:,:)              !  Constituents
    real(r8), pointer :: du3sdtxy(:,:,:)  !! C. Bardeen
    real(r8), pointer :: dv3sdtxy(:,:,:)  !! C. Bardeen
    real(r8), pointer :: dptdtxy(:,:,:)   !! C. Bardeen
...
! Pull the variables out of the dynamics export container
    ...
    v3sxy   => dyn_in%v3s
    t3xy    => dyn_in%t3
    du3sdtxy  => dyn_in%du3sdt   !! C. Bardeen
    dv3sdtxy  => dyn_in%dv3sdt   !! C. Bardeen
    ptxy    => dyn_in%pt
    dptdtxy => dyn_in%dptdt    !! C. Bardeen
...
!!          tsize = 4 + pcnst  !! C. Bardeen
          tsize = 5 + pcnst    !! C. Bardeen
...
!$omp parallel do private (lchnk, ncol, i, k, m, cpter)
          do lchnk = begchunk,endchunk
             ...
             do i=1,ncol
!!                cbuffer(cpter(i,0):cpter(i,0)+3+pcnst) = 0.0_r8  !! C. Bardeen
                cbuffer(cpter(i,0):cpter(i,0)+4+pcnst) = 0.0_r8  !! C. Bardeen
             end do
...
!dir$ concurrent
                do i=1,ncol
                   ...
                   cbuffer(cpter(i,k)+4) = phys_tend(lchnk)%dtdt(i,k)  !! C. Bardeen

                   do m=1,pcnst
!!                      cbuffer(cpter(i,k)+3+m) = phys_state(lchnk)%q(i,k,m)  !! C. Bardee
                      cbuffer(cpter(i,k)+4+m) = phys_state(lchnk)%q(i,k,m)  !! C. Bardeen
                   end do
...
!dir$ concurrent
                do i=ifirstxy,ilastxy
                   ...
                   dptdtxy(i,j,k) = bbuffer(bpter(ib,k)+4) !! C. Bardeen

                   do m=1,pcnst
!!                      tracer(i,j,k,m) = bbuffer(bpter(ib,k)+3+m) !! C. Bardeen
                      tracer(i,j,k,m) = bbuffer(bpter(ib,k)+4+m) !! C. Bardeen
                   end do
...
! -------------------------------------------------------------------------
       ...
       if (iam .lt. grid%npes_xy) then
!!          call uv3s_update( grid, dudtxy, u3sxy, dvdtxy, v3sxy, dt5 )  !! C.Bardeen
         call uv3s_update( grid, dudtxy, u3sxy, du3sdtxy, dvdtxy, v3sxy, dv3sdtxy, dt5 ) !! C.Bardeen
       end if  ! (iam .lt. grid%npes_xy)
...
!!                       delpxy, ptxy, pexy, psxy, ptop)       !! C.Bardeen
                         delpxy, ptxy, dptdtxy, pexy, psxy, ptop)  !! C.Bardeen
    end if  ! (iam .lt. grid%npes_xy)
