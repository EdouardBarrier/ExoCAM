       ...
       do i = 1, ncol
           do k = 1, pverp
             ...
             if (state%zi(i,k) > pblh(i)) then
               ! In the upper atmosphere?
               if (state%pint(i,k) <= kv_top_pressure) then
                 kvh(i,k) = kvh(i,k) * (kv_top_pressure/state%pint(i,k))**kv_top_scale
                 kvm(i,k) = kvm(i,k) * (kv_top_pressure/state%pint(i,k))**kv_top_scale
                 kvq(i,k) = kvq(i,k) * (kv_top_pressure/state%pint(i,k))**kv_top_scale
               else
                 kvh(i,k) = kvh(i,k) * kv_freetrop_scale
                 kvm(i,k) = kvm(i,k) * kv_freetrop_scale
                 kvq(i,k) = kvq(i,k) * kv_freetrop_scale
